 ...
 */
package org.esupportail.portlet.prototyping.maven;
...
import org.esupportail.portlet.util.ReflectionWrapper;
 ...
 * <a href="http://www.eclipse.org/jetty/documentation/current/jetty-maven-plugin.html"><em>jetty-maven-plugin</em></a>.
 *
 * @extendsPlugin jetty
 * @goal run
 * @description Runs the portlet or portlets being developed from the Maven source project
 */
public class PortletPrototypingRunMojo extends JettyRunMojo {

	/** Jetty-Pluto group identifier */
	protected static final String PORTLET_PROTOTYPING_GROUP_ID = "org.esupportail";
	...
	protected static final String VERSION_PROPERTIES_PATH = "/org/esupportail/portlet/prototyping/maven/version.properties";
	
	/** Minimum supported Java version */
	protected static final int MIN_JAVA_VERSION = 7;
	 ...
	 * 
	 * @parameter property="project.build.directory/pluto-resources/portlet.xml"
	 * @readonly
	 * @required
	 */
     ...
     * 
     * @parameter property="project.build.directory/pluto-resources/web.xml"
     * @readonly
     * @required
     */
	 ...
	 * 
	 * @parameter property="portletNames"
	 */
	 ...
	 * 
	 * @parameter property="disableOtherPortlets" default-value="false"
	 */
	...
	protected void checkJavaVersion() {
		...
		if (skip == null && versionStr != null) {
			try {
				...
				int major = Integer.valueOf(versionStr.substring(0, stop1));
				int stop2 = versionStr.indexOf('.', stop1 + 1);
				int minor = Integer.valueOf(versionStr.substring(stop1 + 1, stop2 != -1 ? stop2 : versionStr.length()));
				if (major < 1 || (major == 1 && minor < MIN_JAVA_VERSION)) {
					getLog().error(MessageFormat.format(
							...
							new Object[] { MIN_JAVA_VERSION, versionStr }));
				...
				getLog().warn(MessageFormat.format("Exception parsing Java version string {0}", versionStr), t);
			}
		}
	}
	...
	protected String getDefaultPortletNames() throws MojoExecutionException {
		final PortletXml doc = getParsedPortletXml();
		final Set<String> namesSet = doc.getPortletNames();
		final StringBuilder namesBuf = new StringBuilder();
	...
	protected void assemblePortlets() throws MojoExecutionException {
		...
		if (disableOtherPortlets) {
			final String[] pna = portletNames.split(",");
			final Set<String> portletNamesSet = new HashSet<>(pna.length);
            Collections.addAll(portletNamesSet, pna);
			final PortletXml doc = getParsedPortletXml();
	...
	protected void configureClassPath() throws MojoExecutionException {
        ...
        for (Library portalLibrary : portalLibraries) {
            ...
            String jar = (portalLibrary).getFile();
            ...
            if (cl != null && cl instanceof URLClassLoader) {
                ...
                try {
                    ...
                    throw new MojoExecutionException(MessageFormat.format("Failed to add jar {0} to the path of the class loader using reflection", jar));
                }
            } else {
	...
	protected ContextHandler createPlutoContextHandler() {
		...
		getLog().info(MessageFormat.format("Portal context path = {0}", plutoContextPath));
		getLog().info(MessageFormat.format("Portal WAR = {0}", portal.getFile()));
		// Create context handler
		final WebAppContext plutoHandler = new WebAppContext();
        ...
        for (User user : users)
            loginService.update(
                    ...
                    user.getRolesSet().toArray(new String[]{}));
        plutoHandler.getSecurityHandler().setLoginService(loginService);
		return plutoHandler;
	}
    ...
    protected String resolveArtifact(Artifact artifact) throws MojoExecutionException {
    	try {
    		...
    		throw new MojoExecutionException(MessageFormat.format("Could not resolve artifact {0}", artifact), e);
    	}
